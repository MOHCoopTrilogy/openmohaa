...
void CM_LoadMap( const char *name, qboolean clientload, int *checksum ) {
	...
	*checksum = last_checksum;
	...
	// HZM coop [user 2026-09-01, bug-2314] RETYPE A SURFACE THAT WAS BAKED WRONG IN 2001.
	//
	// "can we ensure the bullet ricochets that hit the tank traps also have the same spark effects
	// and sounds if they hit the higgins boats and tanks".
	//
	// They could not, and no amount of shader work would have done it. The chain:
	//   * the tank traps are RENDER-ONLY static models - there is no static-model collision anywhere
	//     in qcommon/ or server/ - so a bullet actually strikes a clip brush the mapper wrapped
	//     around each one (textures/common/nodraw, contents 0x280e2000 = FENCE|MONSTERCLIP|
	//     WEAPONCLIP|VEHICLECLIP, and notably NO solid bit);
	//   * that brush's surface type comes from the BSP's own baked shader lump, in CMod_LoadShaders
	//     at the top of this file - NOT from any .shader parsed at runtime - so it cannot be
	//     overridden from the mod pk3, by either renderer;
	//   * cgame picks the impact effect from trace.surfaceFlags & MASK_SURF_TYPE
	//     (cg_parsemsg.cpp:83) and needs SURF_METAL/ROCK/GRILL for sparks and the ricochet whine
	//     (cg_parsemsg.cpp:96). The trap clips carry none of them.
	//
	// So the only honest place to change it is here, at clipmap load, in the same spirit as the
	// brush-neutralising patch above. cmpatch/<map>_metal.txt lists brush indices; every SIDE of each
	// one gets SURF_METAL OR'd in. The list is generated by docs/tools/gen_metal_brushes.py, which
	// matches brushes against the LUMP_STATICMODELDEF origins of the steel obstacles - so it is
	// derived from the map rather than hand-picked, and regenerates for any map.
	//
	// SIDES, not cm.brushes: cm_trace.c reads leadside->surfaceFlags (:678, :758) and a cbrush_t has
	// no surfaceFlags of its own. OR rather than assign, so nothing already on the side - a nodraw or
	// nomarks bit the mapper wanted - is discarded.
	{
		char        patchname[MAX_QPATH];
		char        base[MAX_QPATH];
		const char *slash;
		...
		COM_StripExtension( base, base, sizeof( base ) );
		{
			// the server loads maps/<name>_sml.bsp; both variants share one brush table
			int blen = strlen( base );
			if ( blen > 4 && !Q_stricmp( base + blen - 4, "_sml" ) ) {
				base[blen - 4] = 0;
			}
		}
		Com_sprintf( patchname, sizeof( patchname ), "cmpatch/%s_metal.txt", base );

		len = FS_ReadFile( patchname, (void **)&buf );
		if ( len > 0 && buf ) {
			int         applied = 0, skipped = 0, nsides = 0;
			const char *p = buf;
			while ( *p ) {
				while ( *p && ( *p == ' ' || *p == '\t' || *p == '\r' || *p == '\n' || *p == ',' ) ) p++;
				if ( *p == '#' ) {
				...
				if ( !*p ) break;
				{
					int idx = atoi( p );
					if ( idx >= 0 && idx < cm.numBrushes ) {
						cbrush_t *bp = &cm.brushes[idx];
						int       k;
						for ( k = 0; k < bp->numsides; k++ ) {
							bp->sides[k].surfaceFlags |= SURF_METAL;
							nsides++;
						}
						applied++;
					} else {
			...
			Com_Printf( "^~^~^ CMPATCH %s: %d brushes retyped METAL (%d sides), %d out-of-range\n",
				patchname, applied, nsides, skipped );
			FS_FreeFile( buf );
		}
	}
