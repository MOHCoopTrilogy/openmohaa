...
void R_ShutdownFont() {
    ...
    // HZM gl2 (#77): the counters were never reset, so after a vid_restart R_LoadFont
    // could "find" a zeroed cache entry by name ("" match impossible, but the slots were
    // still counted) and, worse, keep appending past MAX_LOADED_FONTS across restarts.
    s_numLoadedFonts = 0;
...
void R_DrawString_sgl(fontheader_sgl_t* font, const char* text, float x, float y, int maxlen, const float *pvVirtualScreen) {
    ...
    // HZM gl2 (bug #73 secondary defect, workflow-verified): this immediate path never
    // bound an FBO - after any FastBlit leaves framebuffer 0 bound, text landed in the
    // BACKBUFFER instead of tr.renderFbo. C shim (linkage) = Draw_StretchPic's preamble.
    R_BindRenderFbo();
